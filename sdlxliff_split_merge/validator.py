...
class SdlxliffValidator:
    ...
    def validate(self, xml_content: str) -> Tuple[bool, Optional[str]]:
        try:
            ...
            if not self._check_critical_tags_balance(xml_content):
                return False, "Обнаружены непарные критические теги"
    ...
    def _check_critical_tags_balance(self, xml_content: str) -> bool:
        critical_pairs = [
            ('xliff', r'<xliff[^>]*>', r'</xliff>'),
            ('file', r'<file[^>]*>', r'</file>'),
            ('header', r'<header[^>]*>', r'</header>'),
            ('body', r'<body[^>]*>', r'</body>')
        ]

        for tag_name, open_pattern, close_pattern in critical_pairs:
            open_count = len(re.findall(open_pattern, xml_content, re.IGNORECASE))
            close_count = len(re.findall(close_pattern, xml_content, re.IGNORECASE))
